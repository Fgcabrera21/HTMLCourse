...
import { a11yDark, a11yLight, agate, atomOneDark, github, githubGist, lioshi, monokai, monokaiSublime, qtcreatorDark } from 'react-syntax-highlighter/dist/esm/styles/hljs';
...
const loadDocs = async (slug: string) => {
    const data = await fetch(process.env.URL + "/mds/" + slug + ".md", { cache: 'no-store' });
...
const Docs = async (props: DocsProps) => {
    ...
    const addId = async (props: any) => {
        ...
        return React.createElement(props.node.tagName, { id: slugify(slug) }, props.children)
    }
    return (
        <div className="flex">
                        ...
                        components={{
                            ...
                            code(props) {
                                ...
                                return match ? (
                                    <SyntaxHighlighter
                                        PreTag="div"
                                        language={match[1] === 'html' ? 'html' : match[1]}
                                        style={a11yDark}
            ...
            <div className="border-l border-gray-100 dark:border-gray-700 h-[calc(100vh-70px)] sticky top-[70px]">
